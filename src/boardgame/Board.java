...
public class Board {
    ...
    public Piece piece(int row, int column) {
        if (!positionExists(row, column)) throw new BoarderException(notPositionOnBoard());
        return pieces[row][column];
    }

    public Piece piecePosition(Position position) {
        if (!positionExists(position)) throw new BoarderException(notPositionOnBoard());
        return pieces[position.getRow()][position.getColumn()];
    }

    public void placePiece(Piece piece, Position position) {
        if (thereIsAPiece(position)) throw new BoarderException(existPieceInTheBoard() + position);
