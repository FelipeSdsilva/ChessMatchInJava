...
public class King extends ChessPiece {
    ...
    public boolean[][] possibleMoves() {
        ...
        if (getMoveCount() == 0 && !chessMatch.getCheck()) {
            Position posTsideKing = new Position(position.getRow(), position.getColumn() + 3);
            if (testRookCastling(posTsideKing)) {
        ...
        if (getMoveCount() == 0 && !chessMatch.getCheck()) {
            Position posTsideQueen = new Position(position.getRow(), position.getColumn() - 4);
            if (testRookCastling(posTsideQueen)) {
                Position p1 = new Position(position.getRow(), position.getColumn() - 1);
                Position p2 = new Position(position.getRow(), position.getColumn() - 2);
                Position p3 = new Position(position.getRow(), position.getColumn() - 3);
                if (getBoard().piecePosition(p1) == null && getBoard().piecePosition(p2) == null
                        && getBoard().piecePosition(p3) == null) {
                    mat[position.getRow()][position.getColumn() - 2] = true;
                }
            }
        }
